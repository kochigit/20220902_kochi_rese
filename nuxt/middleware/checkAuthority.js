export default async function ({ store, redirect, route }) {
  ...
  // storeは改ざんされてしまうので以下でapi側に権限の取得を任せる。上で一度userか見たのはapiへのトラフィックを減らすため。
  // userに対してはlaravelから403や404を投げるとコンソールにapiリクエストのURLが表示されてしまって良くないので204を返し、nuxtではtry catchを使わずに処理する。
  // できればブラウザのコンソールにエラーが表示されないならlaravelから404か403を返したい。できそうな感じはするが工数的にペンディング。
