      ...
      <div class="management" v-for="management in managements" :key="management.id">
        <table class="management__table">
          <tr>
            <th>店舗</th>
            <td>
              <NuxtLink :to="'/detail/'+management.restaurant.uuid" target="_blank" class="management__table__name to-restaurant">{{management.restaurant.name}}</NuxtLink>
...
export default {
  ...
  methods: {
    ...
    async getManagements() {
      ...
      this.approved = gotData.data.managements.filter(mng => mng.approved_at).sort((a, b) => (a.approved_at > b.approved_at) ? -1 : 1);
    },
