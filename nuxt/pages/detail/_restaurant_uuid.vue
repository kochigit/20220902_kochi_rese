    ...
    <validation-observer ref="obs" v-slot="ObserverProps" class="reservation">
      <div class="reservation-form">
        ...
        <validation-provider v-slot="{ errors }" rules="required" class="vp">
          ...
          <input id="date" type="date" name="予約日" v-model="date" class="reservation__input"
            :min="$dayjs().format('YYYY-MM-DD')" />
          <p class="error--orange" v-if="errors[0]">※{{ errors[0] }}</p>
        </validation-provider>
        <validation-provider v-slot="{ errors }" rules="required" class="vp">
          <label for="time">時刻：</label>
          <input id="time" type="time" name="予約時刻" v-model="time" class="reservation__input w-100 time" />
          <p class="error--orange" v-if="errors[0]">※ {{ errors[0] }}</p>
        </validation-provider>
        <validation-provider v-slot="{ errors }" rules="required|min_value:1|max_value:20|integer" class="vp">
          <label for="number">人数：</label>
          <input id="number" type="number" name="人数" v-model="number" class="reservation__input w-100 number" min="1"
            max="20" />
          <p class="error--orange" v-if="errors[0]">※ {{ errors[0] }}</p>
        </validation-provider>
      ...
      <button class="reserve-button" @click="reserve" :disabled="ObserverProps.invalid || !ObserverProps.validated">
        予約する
      </button>
    </validation-observer>
  </div>
