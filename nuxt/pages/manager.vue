      ...
      <transition-group name="card" tag="div">
        <div class="restaurant-tile" v-for="management in managedRestaurants" :key="management.id"
          ...
          <div class="tile-info">
            ...
            <NuxtLink :to="'/manage/' + management.restaurant_uuid" class="to-restaurant-for-manager">予約確認・店舗情報編集ページへ
            </NuxtLink>
          </div>
        </div>
      </transition-group>
        ...
        <div class="create-restaurant-modal" v-if="isShow" @click.self="toggleModal">
          <div class="create-restaurant-info">
            <validation-observer ref="obs" v-slot="ObserverProps">
              ...
              <button @click="createRestaurant" class="create-button"
                :disabled="ObserverProps.invalid || !ObserverProps.validated">店舗を新規作成する</button>
            </validation-observer>
          </div>
        </div>
      </transition>
    ...
    <div class="unmanaged-restaurants">
      ...
      <transition name="fade">
        <div v-show="unmanagedActive">
          <transition-group name="card" tag="div">
            <div class="restaurant-tile" v-for="restaurant in restaurants" :key="restaurant.uuid"
  ...
  export default {
    ...
    methods: {
      ...
      async getRestaurants() {
        ...
        this.restaurants = gotData.data.restaurants.filter(rst => !rst.managements.some((mng) => mng.user_uuid ===
          this.$auth.user.uuid));
      },
